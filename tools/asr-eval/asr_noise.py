...
json.dump(raw, open(os.path.join(W, "noise_raw.json"), "w"))
json.dump(EL.adapt_to_transcribe_json(raw), open(os.path.join(W, "noise_transcribe_shaped.json"), "w"))
json.dump({"elapsed_sec": round(el, 2), "keyterms": len(keyterms), "new_terms": NEW},
          open(os.path.join(W, "noise_timing.json"), "w"), indent=1)
print("done")
