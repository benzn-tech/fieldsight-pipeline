...
def _invoke(function, payload):
    with open(_scratch("_probe.json"), "w", encoding="utf-8") as fh:
        json.dump(payload, fh)
    r = subprocess.run(
        ["aws", "lambda", "invoke", "--function-name", function,
         "--cli-binary-format", "raw-in-base64-out", "--payload", "file://" + _scratch("_probe.json"),
         _scratch("_probe_out.json"), "--region", REGION],
        capture_output=True, text=True)
    if r.returncode:
        print("INVOKE FAILED:", r.stderr[:400], file=sys.stderr)
        return None
    with open(_scratch("_probe_out.json"), encoding="utf-8") as fh:
        return json.load(fh)
